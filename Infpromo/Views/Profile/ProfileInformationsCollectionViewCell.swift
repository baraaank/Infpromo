...
class ProfileInformationsCollectionViewCell: UICollectionViewCell, UITextFieldDelegate, ProfileInformationCollectionReusableViewDelegate {
    ...
    private let headerLabel: UILabel = {
        let label = UILabel()
        label.attributedText = NSAttributedString(string: "aaa", attributes: [NSAttributedString.Key.font : UIFont.systemFont(ofSize: 18, weight: .semibold), NSAttributedString.Key.foregroundColor : UIColor().infpromo])
        return label
    }()
